Installing Django 5.2 Locally (Windows, Mac, Linux)
===================================================

This document describes how to run Django 5.2 on your own laptop or desktop and expose
it to the internet using <a href="https://localhost.run" target="_blank">localhost.run</a>
so that DJ4E autograders can test your work. This setup works on **Windows**, **Mac**, and **Linux**.

When you complete this, you will have a URL from localhost.run (e.g. `https://xxxxx.lhr.lt`)
that you can submit to the "Install" autograder.
...
1. **Python 3.10, 3.11, 3.12, or 3.13** – <a href="https://www.python.org/downloads/" target="_blank">Download Python</a>
   ...
   - On Mac: `python3` is often pre-installed, or use Homebrew: `brew install python`
   - On Linux: `sudo apt install python3 python3-venv` (Debian/Ubuntu) or your distro's package manager

2. **Git** – <a href="https://git-scm.com/downloads" target="_blank">Download Git</a>
   - On Mac: `brew install git` or Xcode Command Line Tools
   - On Linux: `sudo apt install git` (Debian/Ubuntu)
   - On Windows: Git installs **Git Bash**, which we recommend for running the commands below

3. **SSH** – Used by localhost.run (no signup required)
   - Mac and Linux: SSH is pre-installed
   - Windows: SSH comes with Git for Windows; Windows 10/11 also have OpenSSH built-in

We recommend **Git Bash** on Windows so all commands match Mac/Linux. Open "Git Bash" from the Start menu instead of cmd or PowerShell.
...
If `python3` or `python` is not found, try `py -3` on Windows. If you have multiple Python versions, use the one that is 3.10 or newer (e.g. `python3.12 -m venv .ve52`).
...
We provide a checkup script in dj4e-samples. From Git Bash or Mac/Linux:

    bash ~/dj4e-samples/tools/checkup.sh

On Windows without Git Bash, run the equivalent checks manually (e.g. `python manage.py check`).
...
- On Windows, ensure SSH is available: `ssh -V`
