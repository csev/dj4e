...
yt-dlp --skip-download --dump-json --ignore-errors "$PLAYLIST_URL" > youtube-playlist.jsonl
```
...
2. basename of the media file (folder may be missing in yt-dlp filenames)
3. title comparison (YouTube titles omit the `DJ nn.mm` prefix; that is OK)
