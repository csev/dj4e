console.log('Loading DJ4E autograder endpoint in tool frame');
...
var clickRadio = function(label) {
    var inputs = document.getElementsByTagName('input');
     ...
     for (let i = 0; i < inputs.length; i++) {
            ...
            console.debug('label ',el);
            console.debug('textContent', el.textContent);
...
window.addEventListener(
  "message",
  (event) => {
    console.log('In tool frame, processing event', event);
    if ( event.data.command == 'ping' ) {
        const retval ={'text': event.data.text};
        console.log('Tool frame returning', retval);
