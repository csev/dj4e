<?php
/**
 * Django Girls 06 – dynamic
 * Verifies dynamic data in templates (post list with data).
 * Corresponds to: Dynamic data in templates
 */

require_once __DIR__ . "/../crud/webauto.php";

line_out("Django Girls 06 – dynamic");
?>
<p>
Assignment:
...
<p>
Enter the URL of your Django Girls blog. Add at least one post via admin.
</p>
<?php
nameNote();
...
// Post list with dynamic content (article, .post)
...
if ( $crawler !== false ) {
    ...
    if ( $has_structure ) {
        success_out("Post list with dynamic structure");
        $passed++;
    } elseif ( $has_content ) {
        success_out("Page loads (add posts via admin to see dynamic data)");
        $passed++;
    }
}
